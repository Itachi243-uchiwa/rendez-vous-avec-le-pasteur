  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="font-semibold text-[#1A3068]">Créneaux du jour</h3>
      ...
      <AnimatePresence>
        {showForm && (
          ...
          >
            <Card className="p-4 space-y-3 bg-[#E6EBF5]">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-semibold text-[#1A3068] block mb-1">Début</label>
                  <input
                    type="time"
                    value={startTime}
                    onChange={(e) => setStartTime(e.target.value)}
                    className="w-full px-3 py-2 rounded-lg border border-[#B8C8DF] text-sm outline-none focus:border-[#C9A227]"
                  />
                </div>
                <div>
                  <label className="text-xs font-semibold text-[#1A3068] block mb-1">Fin</label>
                  <input
                    type="time"
                    value={endTime}
      ...
      {slots.length === 0 ? (
        ...
        <div className="space-y-2">
          {slots.map((slot) => (
            <div
              key={slot.id}
              className="flex items-center justify-between px-4 py-3 bg-white rounded-xl border border-[#B8C8DF]"
            >
              <div>
                <p className="text-sm font-semibold text-[#1A3068]">
                  {slot.startTime} – {slot.endTime}
                </p>
                {slot.label && <p className="text-xs text-[#475569]">{slot.label}</p>}
              </div>
              <button
