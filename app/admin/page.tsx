...
function shiftDate(dateStr: string, days: number): string {
  // On ajoute "T12:00:00" pour forcer le milieu de journée et éviter tout saut de fuseau horaire
...
// Fonction utilitaire pour générer un bip sonore sans fichier externe
function playAlarmSound() {
  try {
    ...
    osc1.type = 'sine';
    osc1.frequency.setValueAtTime(587.33, audioCtx.currentTime); // Note Ré5
    ...
    // Deuxième bip légèrement décalé
    setTimeout(() => {
      const osc2 = audioCtx.createOscillator();
      const gain2 = audioCtx.createGain();
      osc2.type = 'sine';
      osc2.frequency.setValueAtTime(880, audioCtx.currentTime); // Note La5
...
export default function AdminPage() {
  ...
  // Stocke l'ID du rendez-vous déjà alerté pour éviter les répétitions en boucle
  const alertedAppointmentId = useRef<string | null>(null);
  ...
  // Surveillance du temps restant pour le rendez-vous actif
  useEffect(() => {
    if (!isToday(date) || appointments.length === 0) return;

    const checkTimer = setInterval(() => {
      const activeNow = appointments.find((a) => a.status === 'active');

      if (activeNow && activeNow.estimatedStartTime) {
        // Calcul du timestamp de fin estimé
        const startTimestamp = typeof activeNow.estimatedStartTime === 'number'
            ? activeNow.estimatedStartTime
            : new Date(activeNow.estimatedStartTime).getTime();

        const endTimestamp = startTimestamp + (activeNow.duration * 60 * 1000);
        const now = Date.now();

        // Si le temps est écoulé et qu'on n'a pas encore alerté pour ce rendez-vous
        if (now >= endTimestamp && alertedAppointmentId.current !== activeNow.id) {
          alertedAppointmentId.current = activeNow.id; // Verrouillage de l'alerte

          // 1. Alerte Sonore
          playAlarmSound();

          // 2. Alerte Vibration (2 vibrations de 400ms espacées de 200ms)
  ...
  // Si aucun rendez-vous n'est actif, on réinitialise le verrou d'alerte
  if (!activeNow && alertedAppointmentId.current !== null) {
    alertedAppointmentId.current = null;
  }
  ...
  return (
      <div className="min-h-screen" style={{ background: '#E6EBF5' }}>
        {/* Header, Bannières, Onglets et contenu restent identiques... */}
        <header
